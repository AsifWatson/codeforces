...
int main()
{
    ...
    while(t--) {
        ...
        int alice, bob = 0, turn = 0, lastTurn;
        for(int i=1; i<=n; ++i) {
            int candy;
            cin >> candy;

            if(i == 1) alice = lastTurn = candy, ++turn;
            else lst.push_back(candy);
        }
        ...
        while(true) {
            if(currentPlayer == "bob") {
                thisTurn += lst.back(), bob += lst.back();
                lst.pop_back();
            }

            if(currentPlayer == "alice") {
                thisTurn += lst.front(), alice += lst.front();
                lst.pop_front();
            }
