...
int main()
{
    ...
    while(t--) {
            int n, tempNow, tempNowLo, tempNowHi;
            cin >> n >> tempNow;

            tempNowHi = tempNowLo = tempNow;

            int time[n], lo[n], hi[n];

            for(int i=0; i<n; ++i) {
                    cin >> time[i] >> lo[i] >> hi[i];
            }

            int timeNow = 0, possible = 1;
            for(int i=0; i<n; ++i) {
                    int timeDif = time[i] - timeNow;
                    timeNow = time[i];

                    int tempCanGoLo = tempNowLo - timeDif;
                    int tempCanGoHi = tempNowHi + timeDif;
                    ...
                    if(lo[i] < tempCanGoLo) {
                            ...
                            if(hi[i] >= tempCanGoLo && hi[i] <= tempCanGoHi) {
                                    tempNowLo = tempCanGoLo;
                                    tempNowHi = hi[i];
                            }
                            if(hi[i] > tempCanGoHi) {
                                    tempNowLo = tempCanGoLo;
                                    tempNowHi = tempCanGoHi;
                            }

                    }

                    if(lo[i] >= tempCanGoLo && lo[i] <= tempCanGoHi) {
                            if(hi[i] >= tempCanGoLo && hi[i] <= tempCanGoHi) {
                                    tempNowLo = lo[i];
                                    tempNowHi = hi[i];
                            }
