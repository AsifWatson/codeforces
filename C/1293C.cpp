...
int main()
{
    ...
    set<int> col, leftDiag, rightDiag;
    ...
    for(int i = 1; i <= q; ++i){
            ...
            if(grid[r][c] == 0){
                    ...
                    if(lava.count({r + 1, c}) == 1 || lava.count({r - 1, c}) == 1){
                            col.insert(c);
                    }

                    if(lava.count({r - 1, c + 1}) == 1 || lava.count({r + 1, c - 1}) == 1){
                            if(r == 1) leftDiag.insert(c + c - 1);
                            if(r == 2) leftDiag.insert(c + c + 1);
                    }

                    if(lava.count({r - 1, c - 1}) == 1 || lava.count({r + 1, c + 1}) == 1){
                            if(r == 1) rightDiag.insert(c + c + 1);
                            if(r == 2) rightDiag.insert(c + c - 1);
                    }
            }
            else if(grid[r][c] == 1){
                    grid[r][c] = 0;
                    ...
                    if(lava.count({r - 1, c + 1}) == 1 || lava.count({r + 1, c - 1}) == 1){
                            if(r == 1) leftDiag.erase(c + c - 1);
                            if(r == 2) leftDiag.erase(c + c + 1);
                    }

                    if(lava.count({r - 1, c - 1}) == 1 || lava.count({r + 1, c + 1}) == 1){
                            if(r == 1) rightDiag.erase(c + c + 1);
                            if(r == 2) rightDiag.erase(c + c - 1);
                    }
            }

            if(col.size() == 0 && leftDiag.size() == 0 && rightDiag.size() == 0){
                    cout << "Yes" << endl;
            }
            else cout << "No" << endl;
    }
