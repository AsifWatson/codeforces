...
int main()
{
    ...
    while(t--)
    {
        i64 n, s, canGo = 0, Max = INT_MIN, maxIndex = 0;
        cin >> n >> s;
        ...
        if(canGo == n) cout << "0" << endl;
        else if(canGo == 0) cout << "1" << endl;
        else
        {
            int S = s;
            s += Max;

            int temp1 = canGo - 1, temp2 = canGo;
            for(int i = canGo + 1; i <= n; ++i)
            {
                if(s >= time[i])
                {
                    s -= time[i];
            ...
            if(temp1 >= temp2) cout << maxIndex << endl;
            else cout << canGo + 1 << endl;
        }
    }
