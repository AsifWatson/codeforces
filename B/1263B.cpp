...
#define SortedUnique(v)       sort(All(v)),v.erase(unique(All(v)),v.end())
...
int main()
{
    ...
    while(T--)
    {
        ...
        for(int i = 0; i < N; i++)
        {
            ...
            PinMap[Pin].push_back(i);
            if(PinOnce.count(Pin) == 0) Pins.push_back(Pin);
            PinOnce.insert(Pin);
        }
        ...
        for(auto Pin : Pins)
        {
            int Index = 0;
            char LastChar = Pin[3];
            Output[PinMap[Pin][Index++]] = Pin;
            Taken[Pin] = true;

            for(char Ch = '0'; Ch <= '9'; Ch++)
            {
                if(Index == PinMap[Pin].size()) break;
                if(Ch == LastChar) continue;

                string TempPin(Pin);
                TempPin[3] = Ch;

                if(!Taken[TempPin] && PinOnce.count(TempPin) == 0)
                {
                    Output[PinMap[Pin][Index++]] = TempPin;
                    Taken[TempPin] = true;
                }
            }
        }

        cout << N - PinOnce.size() << endl;
        for(auto Pin : Output) cout << Pin << endl;
    }
