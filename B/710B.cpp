...
int main()
{
    ...
    i64 RightDistance = 0LL, LeftDistance = 0LL;
    for(int i = 1; i < N; i ++)
    {
        RightDistance += abs(Points[i] - Points[0]);
    }

    i64 MinDistance = RightDistance;
    int MinIndex = 0;

    for(int i = 1; i < N; i ++)
    {
        LeftDistance += (Points[i] - Points[i - 1]) * i;
        RightDistance -= (Points[i] - Points[i - 1]) * (N - i);

        if(LeftDistance + RightDistance < MinDistance)
        {
            MinDistance = LeftDistance + RightDistance;
            MinIndex = i;
        }
    }

    cout << Points[MinIndex] << endl;
