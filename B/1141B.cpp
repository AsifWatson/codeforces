...
int main()
{
    ...
    for(int i = 0; i < n; i ++)
    {
        ...
        if(i == n - 1) Last = x;
        if(x == 1) Count++;
        if(x == 0) Rest.push_back(Count), Count = 0;
    }
    if(Count > 0) Rest.push_back(Count);

    int Circular = 0;
    if(Rest.size() > 1 && First == 1 && Last == 1) Circular = (Rest[0] + Rest.back());

    cout << max(Max(Rest), Circular) << endl;
