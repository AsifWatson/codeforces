...
int main()
{
    ...
    while(t--) {
            ...
            int arr[n+5], okForward[n+5], okBackward[n+5];

            Mem(okForward, 0, n+5);
            Mem(okBackward, 0, n+5);

            for(int i=1; i<=n; ++i) {
                    cin >> arr[i];
            }

            set<int> stForward, stBackward;

            for(int i=1; i<=n; ++i) {
                    stForward.insert(arr[i]);

                    if(stForward.size() == i && *stForward.begin() == 1 && *stForward.rbegin() == i) {
                            okForward[i] = 1;
                    }
            }
            ...
            int j = 1;
            for(int i=n; i>=1; --i) {
                    stBackward.insert(arr[i]);

                    if(stBackward.size() == j && *stBackward.begin() == 1 && *stBackward.rbegin() == j) {
                            okBackward[i] = 1;
                    }
            ...
            for(int i=1; i<n; ++i) {
                    if(okForward[i] == 1 && okBackward[i+1] == 1) {
                            ++ans;
                            Ans.push_back({i, n-i});
                    }
            }
